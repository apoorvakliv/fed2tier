...
class fedadagrad():
    ...
    def aggregate(self,server_state_dict,state_dicts):
        ...
        for key, state in zip(keys, self.state):
            state.data = state.data.to(avg_delta_y[key].device).float()
            state.data += torch.square(avg_delta_y[key])
            server_state_dict[key] = server_state_dict[key].to(avg_delta_y[key].device).float()
            server_state_dict[key] = server_state_dict[key].float() + self.lr * avg_delta_y[key] / torch.sqrt(state.data + self.epsilon)
