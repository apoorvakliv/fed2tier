...
class scaffold():
    ...
    def aggregate(self,server_model_state_dict, control_variate, state_dicts, updated_control_variates):
        ...
        for key in keys:
            server_model_state_dict[key] = server_model_state_dict[key].float() + self.lr * delta_x[key].to(server_model_state_dict[key].device)

        control_variate_list = list(range(len(control_variate)))
        for i in control_variate_list:
            control_variate[i] += self.fraction * delta_c[i].to(control_variate[i].device)
