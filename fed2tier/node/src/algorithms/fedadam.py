...
class fedadam():
    ...
    def aggregate(self,server_state_dict,state_dicts):
        ...
        for key, m, v in zip(keys, self.m, self.v):
            m.data = self.beta1 * m.data.to(avg_delta_y[key].data.device) + (1 - self.beta1) * avg_delta_y[key].data
            v.data = self.beta2 * v.data.to(avg_delta_y[key].data.device) + (1 - self.beta2) * torch.square(avg_delta_y[key].data)
            m_bias_corr = m / (1 - self.beta1**self.timestep)
            v_bias_corr = v / (1 - self.beta2**self.timestep)
            server_state_dict[key].data = server_state_dict[key].data.to(avg_delta_y[key].device)
            server_state_dict[key].data = server_state_dict[key].data.float() + self.lr * m_bias_corr / (torch.sqrt(v_bias_corr) + self.epsilon)
